(función (global, fábrica) {
	...
	if (typeof module === "object" && typeof module.exports === "object") {
		// Para entornos tipo CommonJS y CommonJS donde hay una ventana adecuada,
		// ejecuta la fábrica y obtén jQuery
		// Para entornos que no poseen inherentemente una ventana con un documento
		// (como Node.js), exponga una fábrica de creación de jQuery como module.exports
...
// No puedo hacer esto porque varias aplicaciones, incluido el rastreo de ASP.NET
// la pila a través de argumentos.caller.callee y Firefox muere si
// intenta rastrear las cadenas de llamadas "use estrictas". (# 13335)
// Soporte: Firefox 18+
//
...
var
	...
	fcamelCase = function (todo, letra) {
...
jQuery.fn = jQuery.prototype = {
	...
	cada: función (devolución de llamada, argumentos) {
	...
	para (; i <longitud; i ++) {
		...
		if ((opciones = argumentos [i])! = nulo) {
			...
			para (nombre en opciones) {
				...
				if (profundo && copy && (jQuery.isPlainObject (copy) || (copyIsArray = jQuery.isArray (copy)))) {
					if (copyIsArray) {
						copyIsArray = false;
...
jQuery.extend ({
	...
	// Desde la versión 1.3, métodos DOM y funciones como alert
	...
	isNumeric: function (obj) {
		// parseFloat NaNs numeric-cast falsos positivos (nulo | verdadero | falso | "")
	...
	isPlainObject: function (obj) {
		...
		tratar {
			// La propiedad del constructor no debe ser Object
			if (obj.constructor &&
		...
		// Soporte: IE <9
		// Maneja la iteración sobre propiedades heredadas antes de propiedades propias.
		if (support.ownLast) {
			para (clave en obj) {
		...
		// Las propiedades propias se enumeran en primer lugar, así que para acelerar,
	...
	// Microsoft olvidó anotar el prefijo de su proveedor (# 9572)
	...
	cada: función (obj, callback, args) {
		valor var,
			i = 0,
			longitud = obj.length,
		...
		// Un estuche especial y rápido para el uso más común de cada
		} más {
			if (isArray) {
				para (; i <longitud; i ++) {
	...
	fusionar: función (primero, segundo) {
		...
		// Soporte: IE <9
		// Solución alternativa de conversión de .length a NaN en objetos de tipo matriz (p. Ej., NodeLists)
		if (len! == len) {
	...
	proxy: función (fn, contexto) {
		...
		// esto arroja un TypeError, pero solo volveremos indefinido.
		...
		proxy = function () {
			return fn.apply (contexto || this, args.concat (slice.call (argumentos)));
		};

		// Establezca la guía del controlador único en la misma que la del controlador original, para que pueda eliminarse
...
var i,
	...
	push = arr.push,
	slice = arr.slice,
	// Use un índice despojado si no podemos usar uno nativo
	...
	booleans = "verificado | seleccionado | asíncrono | enfoque automático | reproducción automática | controles | diferir | deshabilitado | oculto | ismap | bucle | múltiple | abierto | solo lectura | requerido | alcance",
	...
	rattributeQuotes = new RegExp ("=" + espacios en blanco + "* ([^ \\] '\"] *?) "+ espacios en blanco +" * \\] "," g "),
	...
	matchExpr = {
		...
		"CLASE": nuevo RegExp ("^ \\. (" + CharacterEncoding + ")"),
...
función Sizzle (selector, contexto, resultados, semilla) {
	...
	if (documentIsHTML &&! seed) {
		...
		if ((match = rquickExpr.exec (selector))) {
			// Acelerar: chisporroteo ("# ID")
			if ((m = partido [1])) {
				if (nodeType === 9) {
					elem = context.getElementById (m);
					// Marque parentNode para detectar cuándo vuelve Blackberry 4.6
			...
			// Acelerar: chisporroteo ("TAG")
			} else if (partido [2]) {
				push.apply (results, context.getElementsByTagName (selector));
				devolver resultados;

			// Acelerar: chisporroteo (". CLASE")
		...
		if (support.qsa && (! rbuggyQSA ||! rbuggyQSA.test (selector))) {
			...
			// qSA funciona de forma extraña en consultas con elementos arraigados
			// Podemos solucionar este problema especificando una ID adicional en la raíz
 ...
 * nombre de propiedad de la cadena (con sufijo de espacio) y (si el caché es más grande que Expr.cacheLength)
...
función createPositionalPseudo (fn) {
	return markFunction (function (argumento) {
		...
		return markFunction (función (semilla, coincidencias) {
			...
			// Hacer coincidir elementos encontrados en los índices especificados
			mientras yo-- ) {
...
isXML = Sizzle.isXML = function (elem) {
	...
	documento de retorno documentElement.nodeName! == "HTML": falso;
};
...
setDocument = Sizzle.setDocument = function (nodo) {
	...
	// Si no hay ningún documento y documentElement disponible, devolver
	...
	// Soporte: IE> 8
	// Si el documento de marco flotante se asigna a la variable "documento" y si se ha recargado el marco flotante,
	// IE arrojará el error "permiso denegado" cuando acceda a la variable "documento", consulte jQuery # 13936
	...
	support.attributes = afirmar (función (div) {
		div.className = "i";
	...
	support.getElementsByTagName = afirmar (función (div) {
		div.appendChild (doc.createComment (""));
		return! div.getElementsByTagName ("*"). length;
	});

	// Comprueba si se puede confiar en getElementsByClassName
	support.getElementsByClassName = rnative.test (doc.getElementsByClassName) && afirmar (función (div) {
		div.innerHTML = "<div class = 'a'> </div> <div class = 'a i'> </div>";

		// Soporte: Safari <4
		// Captura de almacenamiento en caché de clase
	...
	support.getById = afirmar (función (div) {
	...
	if (support.getById) {
		...
		Expr.filter ["ID"] = función (id) {
			var attrId = id.replace (runescape, funescape);
			función de retorno (elem) {
				var nodo = typeof elem.getAttributeNode! == strundefined && elem.getAttributeNode ("id");
	...
	if ((support.qsa = rnative.test (doc.querySelectorAll))) {
		...
		afirmar (función (div) {
			...
			if (! div.querySelectorAll (": marcado"). length) {
	...
	// A propósito no implementa descendiente inclusivo
	...
	función (a, b) {
		...
		si (compare & 1 ||
			...
			if (a === doc || a.ownerDocument === preferredDoc && contiene (preferredDoc, a)) {
				volver -1;
			}
			if (b === doc || b.ownerDocument === preferredDoc && contiene (preferredDoc, b)) {
				retorno 1;
			}
...
Sizzle.matchesSelector = function (elem, expr) {
	...
	// Asegúrese de que los selectores de atributos están entre comillas
...
Sizzle.error = función (mensaje) {
	lanzar un nuevo error ("Error de sintaxis, expresión no reconocida:" + mensaje);
};

/ **
 * Clasificación y eliminación de documentos duplicados.
...
Sizzle.uniqueSort = function (resultados) {
	...
	// A menos que * sepamos * que podemos detectar duplicados, asuma su presencia
 ...
 * /
getText = Sizzle.getText = función (elem) {
	nodo var,
		ret = "",
		i = 0,
	...
	if (! nodeType) {
		...
		// uso de innerText eliminado por consistencia de nuevas líneas (jQuery # 11153)
	...
	// No incluya comentarios o nodos de instrucciones de procesamiento
...
Expr = Sizzle.selectors = {
	...
	preFiltro: {
		"ATTR": función (coincidencia) {
			...
			// Mueve el valor dado para que coincida [3] ya sea entre comillas o sin comillas
		...
		"NIÑO": función (coincidencia) {
			...
			* /
			...
			if (match [1] .slice (0, 3) === "enésimo") {
				...
				// recuerda ese elenco falso / verdadero respectivamente a 0/1
				partido [4] = + (partido [4]? partido [5] + (partido [6] || 1): 2 * (partido [3] === "par" || partido [3] === " impar" ) );
				match [5] = + ((match [7] + match [8]) || match [3] === "impar");
		...
		"PSEUDO": función (partido) {
			...
			// Aceptar argumentos citados tal cual
			if (partido [3]) {
	...
	filtro: {
		...
		"TAG": función (nodeNameSelector) {
			varNameName = nodeNameSelector.replace (runescape, funescape) .toLowerCase ();
		...
		"CLASE": función (className) {
			patrón var = classCache [className + ""];

			patrón de retorno ||
				(patrón = nuevo RegExp ("(^ |" + espacio en blanco + ")" + className + "(" + espacio en blanco + "| $)")) &&
		...
		"ATTR": función (nombre, operador, cheque) {
			función de retorno (elem) {
				resultado var = Sizzle.attr (elem, nombre);
				...
				operador de retorno === "="? resultado === comprobar:
					...
					operador === "* ="? verifique && result.indexOf (verifique)> -1:
		...
		"NIÑO": función (tipo, qué, argumento, primero, último) {
			...
			volver primero === 1 && último === 0?
				...
				función (elem, contexto, xml) {
					var caché, externalCache, nodo, diff, nodeIndex, inicio,
					...
					si (padre) {
						...
						if (reenviar && useCache) {
							// Buscar `elem` de un índice previamente almacenado en caché
							outsideCache = padre [expando] || (padre [expando] = {});
							caché = externalCache [tipo] || [];
						...
						// Use el índice del elemento previamente almacenado en caché si está disponible
						...
						// Incorpora el desplazamiento, luego verifica el tamaño del ciclo
						diff - = último;
		...
		"PSEUDO": función (pseudo, argumento) {
			...
			// Pero mantenga el soporte para firmas antiguas
			if (fn.length> 1) {
				...
				return Expr.setFilters.hasOwnProperty (pseudo.toLowerCase ())?
					markFunction (función (semilla, coincidencias) {
						...
						mientras yo-- ) {
							...
							seed [idx] =! (coincide con [idx] = coincide [i]);
						}
					}):
					función (elem) {
	...
	pseudos: {
		...
		"not": markFunction (function (selector) {
			...
			volver matcher [expando]?
				markFunction (function (semilla, coincidencias, contexto, xml) {
					var elem,
				...
				función (elem, contexto, xml) {
					entrada [0] = elem;
					matcher (input, null, xml, resultados);
					return! results.pop ();
				};
		}),
		...
		// La comparación de C con el valor del lenguaje del elemento se realiza sin distinción entre mayúsculas y minúsculas.
		...
		"marcado": función (elem) {
			// En CSS3,: check debería devolver los elementos marcados y seleccionados
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
			var nodeName = elem.nodeName.toLowerCase ();
			return (nodeName === "input" && !! elem.checked) || (nodeName === "opción" && !! elem.selected);
		},

		"selected": función (elem) {
			// Acceder a esta propiedad hace seleccionado por defecto
		...
		"vacío": función (elem) {
			...
			//: vacío está negado por el elemento (1) o los nodos de contenido (texto: 3; cdata: 4; entidad ref: 5),
			// pero no por otros (comentario: 8; instrucción de procesamiento: 7; etc.)
			// nodeType <6 funciona porque los atributos (2) no aparecen como elementos secundarios
			for (elem = elem.firstChild; elem; elem = elem.nextSibling) {
				if (elem.nodeType <6) {
					falso retorno;
				}
		...
		"texto": función (elem) {
			...
			return elem.nodeName.toLowerCase () === "input" &&
				...
				// Soporte: IE <8
				// Los nuevos valores de atributo HTML5 (por ejemplo, "buscar") aparecen con elem.type === "text"
		...
		"eq": createPositionalPseudo (función (matchIndexes, longitud, argumento) {
...
// Añadir pseudos de botón / tipo de entrada
...
tokenize = Sizzle.tokenize = function (selector, parseOnly) {
	...
	while (soFar) {
		...
		if (! coincide || (match = rcomma.exec (soFar))) {
			if (partido) {
		...
		if ((match = rcombinators.exec (soFar))) {
			...
			tokens.push ({
				valor: emparejado,
				// Lanza los combinadores descendientes al espacio
				tipo: partido [0] .replace (rtrim, "")
			});
			soFar = soFar.slice (longitud coincidente);
		}
...
función addCombinator (matcher, combinator, base) {
	...
	volver combinador.primero?
		// Verificar con el ancestro más cercano / elemento anterior
		función (elem, contexto, xml) {
			while ((elem = elem [dir])) {
		...
		// Comprueba todos los elementos ancestros / anteriores
		función (elem, contexto, xml) {
			var oldCache, outsideCache,
			...
			if (xml) {
				...
				while ((elem = elem [dir])) {
					if (elem.nodeType === 1 || checkNonElements) {
						...
						if ((oldCache = outsideCache [dir]) &&
							oldCache [0] === dirruns && oldCache [1] === doneName) {

							// Asignar a newCache para que los resultados se propaguen a elementos anteriores
							return (newCache [2] = oldCache [2]);
						} más {
							// Reutilice newcache para que los resultados se propaguen a elementos anteriores
							outsideCache [dir] = newCache;

							// Una coincidencia significa que hemos terminado; un fallo significa que tenemos que seguir revisando
...
función setMatcher (preFilter, selector, matcher, postFilter, postFinder, postSelector) {
	...
	return markFunction (function (semilla, resultados, contexto, xml) {
		var temp, i, elem,
			preMap = [],
			postMap = [],
			preexistente = resultados.length,
			...
			// Prefiltro para obtener entrada de coincidencia, preservando un mapa para la sincronización de resultados semilla
		...
		if (postFilter) {
			...
			// No coincide los elementos que fallan moviéndolos de nuevo a matcherIn
			i = longitud temporal;
			mientras yo-- ) {
				if ((elem = temp [i])) {
		...
		si (semilla) {
			if (postFinder || preFilter) {
				...
				// Mueve elementos coincidentes de la semilla a los resultados para mantenerlos sincronizados
				i = matcherOut.length;
				mientras yo-- ) {
					if ((elem = matcherOut [i]) &&
...
función matcherFromTokens (tokens) {
	var checkContext, matcher, j,
		...
		implicitRelative = LeadingRelative || Expr.relativo [""],
		...
		}, implícitaRelativo, verdadero),
		matchAnyContext = addCombinator (function (elem) {
			return indexOf.call (checkContext, elem)> -1;
		}, implícitaRelativo, verdadero),
		matchers = [function (elem, context, xml) {
			return (! LeadingRelative && (xml || context! == outsidemostContext)) || (
	...
	para (; i <len; i ++) {
		if ((matcher = Expr.relative [tokens [i] .type])) {
			...
			// Regreso especial al ver un marcador posicional
			if (matcher [expando]) {
	...
	var bySet = setMatchers.length> 0,
		...
		superMatcher = function (semilla, contexto, xml, resultados, más externo) {
			var elem, j, matcher,
				...
				sin igual = semilla && [],
				setMatched = [],
				contextBackup = outsidemostContext,
				// Siempre debemos tener elementos semilla o contexto más externo
				elems = semilla || byElement && Expr.find ["TAG"] ("*", más externo),
				// Usa dirruns enteros si este es el más externo
				dirrunsUnique = (dirruns + = contextBackup == null? 1: Math.random () || 0.1),
				len = elems.length;

			if (exterior) {
				outsidemostContext = context! == document && context;
			}

			// Agregar elementos que pasan elementMatchers directamente a los resultados
			// Mantenga `i` una cadena si no hay elementos, por lo que` matchedCount` será "00" a continuación
			// Soporte: IE <9, Safari
			// Tolerar propiedades de NodeList (IE: "length"; Safari: <number>) elementos coincidentes por id
			for (; i! == len && (elem = elems [i])! = null; i ++) {
				...
				// Seguimiento de elementos no coincidentes para filtros de conjunto
				if (bySet) {
					// Habrán pasado por todos los posibles emparejadores
			...
			if (bySet && i! == matchedCount) {
				...
				while ((matcher = setMatchers [j ++])) {
					matcher (no coincide, setMatched, context, xml);
				}

				si (semilla) {
					// Reintegra las coincidencias de elementos para eliminar la necesidad de ordenar
					if (matchedCount> 0) {
						mientras yo-- ) {
							if (! (sin coincidir [i] || setMatched [i])) {
				...
				// El conjunto sin semillas coincide con múltiples coincidencias exitosas estipulan la clasificación
				if (exterior &&! seed && setMatched.length> 0 &&
			...
			// Anular la manipulación de globales por matizadores anidados
			if (exterior) {
				dirruns = dirrunsUnique;
...
compile = Sizzle.compile = function (selector, match / * Internal Use Only * /) {
	...
	if (! caché) {
		// Genera una función de funciones recursivas que puede usarse para verificar cada elemento
		if (! match) {
			partido = tokenizar (selector);
		}
		i = longitud de partido;
		mientras yo-- ) {
			en caché = matcherFromTokens (partido [i]);
			if (en caché [expando]) {
		...
		en caché = compilerCache (selector, matcherFromGroupMatchers (elementMatchers, setMatchers));
...
select = Sizzle.select = function (selector, contexto, resultados, semilla) {
	var i, tokens, token, tipo, buscar,
		compilado = typeof selector === "función" && selector,
	...
	if (match.length === 1) {
		...
		// Tome un acceso directo y establezca el contexto si el selector raíz es una ID
		...
		if (tokens.length> 2 && (token = tokens [0]). type === "ID" &&
			...
			// Los comparadores precompilados seguirán verificando la ascendencia, así que sube de nivel
			} else if (compilado) {
		...
		mientras yo-- ) {
			...
			if ((find = Expr.find [type])) {
				// Búsqueda, contexto expansivo para los principales combinadores hermanos
				if ((seed = find (
...
// Los nodos separados se siguen confusamente * entre sí *
support.sortDetached = afirmar (función (div1) {
...
// Soporte: IE <9
// Usa getAttributeNode para obtener booleanos cuando getAttribute miente
si (! afirmar (función (div) {
	return div.getAttribute ("disabled") == nulo;
})) {
...
// Implemente la funcionalidad idéntica para filtro y no
...
var rootjQuery,
	...
	// Use el documento correcto de acuerdo con el argumento de la ventana (sandbox)
	document = window.document,

	// Una forma sencilla de buscar cadenas HTML
	// Prioriza #id sobre <tag> para evitar XSS a través de location.hash (# 9521)
	// Reconocimiento HTML estricto (# 11290: debe comenzar con <)
	rquickExpr = / ^ (?: \ s * (<[\ w \ W] +>) [^>] * | # ([\ w -] *)) $ /,
		...
		if (typeof selector === "string") {
			...
			if (match && (match [1] ||! context)) {
				...
				if (partido [1]) {
					...
					// los scripts son verdaderos para back-compat
					// Deja que se produzca el error intencionalmente si parseHTML no está presente
					...
					if (rsingleTag.test (match [1]) && jQuery.isPlainObject (context)) {
						para (coincidencia en contexto) {
							// Las propiedades del contexto se invocan como métodos si es posible
							if (jQuery.isFunction (this [match])) {
								this [partido] (contexto [partido]);

							// ... y de lo contrario establecer como atributos
							} más {
								this.attr (partido, contexto [partido]);
							}
						}
					}
				...
				// MANGO: $ (# id)
					...
					// nodos que ya no están en el documento # 6963
					if (elem && elem.parentNode) {
						// Manejar el caso donde IE y Opera devuelven artículos
						// por nombre en lugar de ID
						if (elem.id! == match [2]) {
...
// Dele a la función init el prototipo jQuery para una instanciación posterior
...
jQuery.fn.extend ({
	...
	más cercano: función (selectores, contexto) {
		var cur,
			...
			l = esta.longitud,
			emparejado = [],
			pos = rneedsContext.test (selectores) || typeof selectors! == "string"?
				jQuery (selectores, contexto || this.context):
				0;

		para (; i <l; i ++) {
			for (cur = this [i]; cur && cur! == context; cur = cur.parentNode) {
				// Siempre omita fragmentos de documentos
				if (cur.nodeType <11 && (pos?
					pos.index (cur)> -1:

					// No pase elementos que no sean elementos a Sizzle
					cur.nodeType === 1 &&
		...
		devuelve this.pushStack (longitud coincidente> 1? jQuery.unique (coincidencia): coincidencia);
	},
	...
	addBack: function (selector) {
		devuelve this.add (selector == nulo?
...
jQuery.each ({
	...
	prevUntil: function (elem, i, hasta) {
		return jQuery.dir (elem, "previousSibling", hasta);
	},
	...
	jQuery.fn [nombre] = función (hasta, selector) {
		var ret = jQuery.map (this, fn, hasta);
		...
		if (this.length> 1) {
			// Eliminar duplicados
			si (! garantizadoUnique [nombre]) {
				ret = jQuery.unique (ret);
			}

			// Orden inverso para padres * y derivadas previas
